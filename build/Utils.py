...
def dummy_inputs(local_batch_size):
    ...
    dummy_inputs = np.random.uniform(-1.0, 1.0, [256, 256]).astype((np.float32))
    dummy_labels = np.random.randint(0, 2, []).astype(np.float32)

    # Create the dataset object. Shuffle, then batch it, then make it repeat indefinitely
    dataset= tf.data.Dataset.from_tensors((dummy_inputs, dummy_labels)).shuffle(200).repeat().batch(local_batch_size)
